...
/* ARR = ความละเอียด PWM: นับ 0-999 (1000 ขั้น) ต่อ 1 รอบ */
#define PWM_ARR_VALUE (999u)

/*Requirement: อ่านค่า ADC จาก PA1 (channel 1) แล้วแปลงเป็น 
duty cycle 0-100% สั่งควบคุมความสว่าง LED ผ่าน PWM ที่ PA6 (TIM3 Channel 1)
 แบบ real-time (วนอ่าน+ปรับ duty ตลอดใน while(1))*/
...
/***********************************************************************
 * @fn      - clock_init
 * @brief   - เปิด clock: GPIOA (PA1 สำหรับ ADC, PA6 สำหรับ PWM),
 *            ADC1 (บัส APB2), TIM3 (บัส APB1)
 ************************************************************************/
static void clock_init(void)
{
...
/***********************************************************************
 * @fn      - adc_init
 * @brief   - ตั้งค่า PA1 เป็น Analog + ตั้งให้ ADC แปลง channel 1
 ************************************************************************/
static void adc_init(void)
{
...
/***********************************************************************
 * @fn      - adc_read
 * @brief   - สั่งแปลงค่า ADC 1 ครั้ง รอจนเสร็จ แล้วคืนค่าผลลัพธ์ (0-4095)
 ************************************************************************/
static uint16_t adc_read(void)
{
...
/***********************************************************************
 * @fn      - pwm_init
 * @brief   - ตั้งค่า PA6 เป็น TIM3_CH1 (Alternate Function) แล้วตั้งค่า
 *            Timer ให้เป็น PWM ความถี่ 1kHz, duty เริ่มต้น 0%
 ************************************************************************/
static void pwm_init(void)
{
...
/***********************************************************************
 * @fn      - pwm_set_duty
 * @brief   - ตั้งค่า duty cycle เป็น % (0-100)
 * @param   - percent : ค่า duty ที่ต้องการ 0-100
 ************************************************************************/
static void pwm_set_duty(uint16_t percent)
{
    if (percent <= 100u)
