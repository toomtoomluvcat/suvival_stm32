...
/*
อ่านค่า ADC จาก PA0 (channel 0) ทุก ๆ 1 วินาที 
แล้วส่งข้อความออกทาง UART ในรูปแบบ "ADC = 1234\r\n" (เลขคือค่าที่อ่านได้จริง)
ถ้ารับตัวอักษร '1' เข้ามาทาง UART (RX) ให้ LED (PA5) ติด ถ้ารับ '0' ให้ดับ
ทำทั้ง 2 อย่างพร้อมกันได้ (ใช้ polling สลับกันไปในลูปเดียว)*/
int main(void)
{
    clock_init();
    led_init();
...
static void adc_init(void)
{
    GPIOA->MODER &= ~GPIO_MODER_MODER0;    /* แก้: เคลียร์ขา 0 ให้ตรงกับที่ตั้งค่า */
    GPIOA->MODER |= (0b11 << (0 * 2));

    ADC1->SMPR2 |= ADC_SMPR2_SMP0;         /* แก้: SMP0 ตรงกับ channel 0 */
